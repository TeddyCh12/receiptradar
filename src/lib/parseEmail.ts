...
export function parseEmailMinimal(raw: string): ParsedEmail {
  ...
  const totalCents = moneyMatch?.[1] ? toCents(moneyMatch[1].replace(/[^\d.,]/g, '')) : undefined;
  ...
  const purchasedAt = dateStr ? new Date(dateStr) : new Date();

  // build without inserting undefined properties
  const out: ParsedEmail = { currency, purchasedAt };
  if (merchant) out.merchant = merchant;
  if (typeof totalCents === 'number') out.totalCents = totalCents;
  return out;
}
